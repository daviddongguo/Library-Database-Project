...
if OBJECT_ID('Books.FindBookByTitleSP', 'P') is not null
	drop procedure Books.FindBookByTitleSP
;
go
create procedure Books.FindBookByTitleSP
(
	@Title as nvarchar(60) = null
)
as
	begin
		select distinct Title, ISBN, Author, Loanable
		from books.TitleDetailedView
		where Title like { fn CONCAT('%', { fn CONCAT(IIF((@Title is null), '%', @Title), '%') }) }
	end
;
go

execute Books.FindBooksSP @Title = 'first'
;
go
